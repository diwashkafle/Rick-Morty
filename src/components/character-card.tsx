...
const CharacterCard = ({
  ...
  return (
    <main className="flex flex-col items-center border border-gray-200 rounded-md">
      <section className="py-7 flex flex-col gap-3 px-1">
        <img className="rounded-md w-48 h-48 sm:w-64 sm:h-64" src={image} alt={name} loading="lazy" />
        <h2 className="text-lg font-semibold flex items-center gap-2">
            {name.length>20 ? name.slice(0,15) + "..." : name}
        <p className="border py-0.5 border-gray-200 flex font-semibold items-center rounded-md bg-gray-50 px-1.5 text-[10px] text-gray-600">{gender}</p> 
        </h2>
        <div className="flex items-center gap-2">
          ...
          <p className="border py-0.5 border-gray-200 flex font-semibold items-center rounded-md bg-gray-50 px-1.5 text-[10px] text-gray-600">Species: {species}</p>
        </div>
      </section>
    </main>
  );
